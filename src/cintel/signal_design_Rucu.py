...
def main() -> None:
    ...
    # ---------------------------------------------------------
    # STEP 2.7: (MODIFICATION) DEFINE THE WEIGHED INDEX FORMULA
    # ---------------------------------------------------------
    load_index_recipe = ((pl.col("requests") * 1.0) + (pl.col("errors") * 5.0)).alias(
        "weighted_load_index"
    )
    ...
    df_with_signals = df_with_signals.with_columns(
        ...
        .when(
            ((pl.col("requests") > 50) & (pl.col("requests") <= 100))
            | ((pl.col("errors") >= 2) & (pl.col("errors") <= 4))
        )
        .then(pl.lit("Medium load index"))
        .otherwise(pl.lit("High load index"))
        .alias("load_index_category")
    )

    LOG.info("Created load index category column : load_index_category")

    # ----------------------------------------------------
    # STEP 3.1: SELECT THE COLUMNS WE WANT TO SAVE
    # ----------------------------------------------------
